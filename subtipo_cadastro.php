...
// Bloqueia se o usuário não tiver permissão "subtipos"
if (!verificaPermissao('subtipos')) {
...
if (isset($_GET['editar'])) {
    $id = (int) $_GET['editar'];
    $stmt = $pdo->prepare("SELECT * FROM subtipos WHERE id = ?");
    $stmt->execute([$id]);
    if ($stmt->rowCount() > 0) {
        $edit = true;
...
if (isset($_GET['excluir'])) {
    $id = (int) $_GET['excluir'];
    $pdo->prepare("DELETE FROM subtipos WHERE id = ?")->execute([$id]);
...
<div class="container py-4 main-container">
    <form method="POST" action="subtipo_salvar.php" class="mb-4">
        <h4 class="mb-4"><?= $edit ? 'Editar Subtipo de Produto' : 'Cadastro de Subtipo de Produto' ?></h4>
    ...
    <div class="listagem">
        <h4>Subtipos de Produtos</h4>

        <form method="GET" class="d-flex gap-2 mb-3">
            <input type="text" name="busca" class="form-control" placeholder="Pesquisar por subtipo..." value="<?= htmlspecialchars($busca) ?>">
            <button class="btn btn-primary" type="submit">Buscar</button>
            <a href="subtipo_cadastro.php" class="btn btn-secondary">Limpar</a>
        </form>
